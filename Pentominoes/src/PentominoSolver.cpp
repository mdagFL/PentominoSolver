...
namespace Pentominoes
{
	...
	void PentominoSolver::removeTrivialSolutions()
	{
		...
		if (solutionsFound->size() > 0)
		{
			...
			{
				// Less simple for square boards, so try each transformation and check if
			    // the board is still in the solution set
				for (int i = 0; i < 7; i++)
				{
	...
	void PentominoSolver::solveBacktracking(int depth)
	{
		...
		else
		{
			...
			{	
				...
				Pentomino nextPiece(static_cast<UniqueOrientation>(i));
	...
	bool PentominoSolver::checkPieceAvailable(const Pentomino& piece) const
	{
		...
		BasePentominoType base{ piece.getBasePiece() };
